dsl = Agent.create(:youtube_launch_optimizer) do
  ...
  prompts do
    prompt :configure_prompt                  , content: load_file("1-1-configure.hbs")
    prompt :title_shortlist_prompt            , content: load_file("1-2-title-shortlist.hbs")
    prompt :video_summary_prompt              , content: load_file("1-3-summarize-video.hbs")
    prompt :video_abridgement_prompt          , content: load_file("1-4-abridge.hbs")
    prompt :abridgement_qa_prompt             , content: load_file("1-5-abridge-qa.hbs")
    prompt :intro_outro_seperation_prompt     , content: load_file("1-6-seperate-intro-outro.hbs")
    prompt :find_video_cta_prompt             , content: load_file("1-7-find-video-cta.hbs")
  ...
  attributes do
    ...
    attribute :title_ideas_shortlist, type: :array
  ...
  section 'Video Preparation' do
    ...
    step 'Title Shortlist' do
      input :title_ideas
      prompt :title_shortlist_prompt
      output :title_ideas_shortlist
      # Pattern: Flexible Selection (human picks from numbered list or adds own)
    end
    ...
    step 'Abridge QA' do
      ...
      prompt :abridgement_qa_prompt
      output :transcript_abridgement_qa
    end

    step 'Intro/Outro Separation' do
      input :transcript
    ...
    step 'Find Video CTA' do
      ...
      output :future_video_cta
      output :past_video_cta
    end
  end
